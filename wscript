    ...
    try:
        for filename in os.listdir("scratch"):
            ...
            if os.path.isdir(os.path.join("scratch", filename)):
                ...
                obj.name = obj.target
                obj.install_path = None
        for filename in os.listdir("testswitchml"):
            if filename.startswith('.') or filename == 'CVS':
                continue
            if os.path.isdir(os.path.join("testswitchml", filename)):
                obj = bld.create_ns3_program(filename, all_modules)
                obj.path = obj.path.find_dir('testswitchml').find_dir(filename)
                obj.source = obj.path.ant_glob('*.cc')
                obj.target = filename
                obj.name = obj.target
                obj.install_path = None
                ...
                name = filename[:-len(".cc")]
                obj = bld.create_ns3_program(name, all_modules)
                obj.path = obj.path.find_dir('testswitchml')
                obj.source = filename
                obj.target = name
                obj.name = obj.target
                obj.install_path = None
