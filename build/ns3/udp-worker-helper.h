 ...
 */
#ifndef UDP_WORKER_HELPER_H
#define UDP_WORKER_HELPER_H
...
public:
  /**
   * Create UdpPsHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpPsHelper (uint16_t port);
  ...
  /**
   * Create a UdpPsApplication on the specified Node.
   *
  ...
  /**
   * Create a UdpPsApplication on specified node
   *
...
private:
  /**
   * Install an ns3::UdpPs on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpPs will be installed.
...
class UdpAggregatorHelper
{
public:
  /**
   * Create UdpAggregatorHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpAggregatorHelper (uint16_t port);
  ...
  /**
   * Create a UdpAggregatorApplication on the specified Node.
   *
   * \param node The node on which to create the Application.  The node is
   *             specified by a Ptr<Node>.
   *
  ...
  /**
   * Create a UdpAggregatorApplication on specified node
   *
   * \param nodeName The node on which to create the application.  The node
   *                 is specified by a node name previously registered with
   *                 the Object Name Service.
...
private:
  /**
   * Install an ns3::UdpAggregator on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpAggregator will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
 ...
 */
class UdpWorkerHelper
{
public:
  /**
   * Create UdpWorkerHelper which will make life easier for people trying
   ...
   */
  UdpWorkerHelper (Address ip, uint16_t port);
  /**
   * Create UdpWorkerHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a UdpWorker application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be UdpWorker).
   ...
   *
   * \param node The Ptr<Node> on which to create the UdpWorkerApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the UdpWorkerApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::UdpWorker on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpWorker will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
