...
//LZYECMP
int Ipv4GlobalRouting::job_ecmp_table[128][128] = {-1};
bool Ipv4GlobalRouting::job_map[128] = {false};
...
Ptr<Ipv4Route>
Ipv4GlobalRouting::LookupGlobal (ns3::Ptr<const Packet> p, Ipv4Header ipv4header, Ipv4Address dest, Ptr<NetDevice> oif)
{
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      bool isSwml = 0;
      uint16_t appid, hostid=1000;
      SwmlRouteTag swmlroutetag;
      if(p->PeekPacketTag(swmlroutetag)){
        
        swmlroutetag.GetSwmlRouteTag(appid, hostid);
        isSwml = 1;
        //printf("yes, [app] %d [host] %d\n",appid,hostid);
      ...
      if (m_randomEcmpRouting)
        {
          ...
          if(isSwml && allRoutes.size () > 1 ){

            // if(job_map[appid] == false){
            //   for (int i=0; i<allRoutes.size (); i++){
            //     job_ecmp_table[appid][i] = i;
            //   }
            //   job_map[appid] = true;
            //   for (int i=0; i<allRoutes.size (); i++){
            //     int ex_index = m_rand->GetInteger (i, allRoutes.size ()-1);
            //     int temp = job_ecmp_table[appid][i];
            //     job_ecmp_table[appid][i] = job_ecmp_table[appid][ex_index];
            //     job_ecmp_table[appid][ex_index] = temp;
            //   }
            //   printf("[Paht of job %d] ",appid);
            //   for (int i=0; i<allRoutes.size (); i++){
            //     printf("%d ",job_ecmp_table[appid][i]);
            //   }
            //   printf("\n");
              
            // }

            // selectIndex = job_ecmp_table[appid][hostid] ;
            selectIndex = m_rand->GetInteger (0, allRoutes.size ()-1); 
              ...
              // }
              // printf("\n");
            //if(job_ecmp_table[0][2] == job_ecmp_table[0][0])
              //printf("app %d host %d select path %d\n",appid, hostid, selectIndex);
            ...
            hash_feild.srcaddr = ipv4header.GetDestination();
            hash_feild.dstaddr = ipv4header.GetSource();

            unsigned long long hash_input1, hash_input2;
            //char hash_input[16];
            memcpy(&hash_input1, &hash_feild.srcaddr, sizeof(ns3::Ipv4Address));
            memcpy(&hash_input2, &hash_feild.dstaddr, sizeof(ns3::Ipv4Address));

            //std::cout<<sizeof(unsigned long long)<<std::endl;
            std::hash<unsigned long long> hash_func; //lzy
            uint32_t hash_value1 = hash_func(hash_input1);
            uint32_t hash_value2 = hash_func(hash_input2);
            uint32_t hash_value = (hash_value1 + hash_value2);
            selectIndex = (hash_value1 + hash_value2) % 997 % 129 % allRoutes.size (); 
            //printf("select path %d\n",selectIndex);
          }
          
          //if (allRoutes.size () == 4)  std::cout<<"src "<<hash_feild.srcaddr<<" dst "<<hash_feild.dstaddr<<" hash_value "<<hash_value<<" selectIndex "<<selectIndex <<std::endl;
          //selectIndex = m_rand->GetInteger (0, allRoutes.size ()-1); 
          //std::cout<<"protocl "<<(uint32_t)ipv4header.GetProtocol()<<" src "<<ipv4header.GetSource()<<std::endl;
      ...
      // if (allRoutes.size () == 4)
      // std::cout<<"size "<<allRoutes.size ()<<" dest "<<dest<<" nodedest "<<m_ipv4->GetAddress (selectIndex, 0) <<" m_ipv4 "<< m_ipv4->GetAddress (interfaceIdx, 0) <<std::endl;
      return rtentry;
    }
  else 
    {
...
//
  ...
  Ptr<Ipv4Route> rtentry = LookupGlobal (p, header, header.GetDestination (), oif);
  if (rtentry)
    {
...
{ 
  ...
  Ptr<Ipv4Route> rtentry = LookupGlobal (p, header, header.GetDestination ());
  if (rtentry != 0)
    {
