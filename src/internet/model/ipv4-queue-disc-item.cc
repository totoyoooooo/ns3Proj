...
Ipv4QueueDiscItem::Print (std::ostream& os) const
{
  ...
  os << GetPacket () << " "
     ...
     << "txq " << (uint16_t) GetTxQueueIndex ()<< std::endl
  ;
}
...
Ipv4QueueDiscItem::Mark (void)
{
  ...
  //for ecn mark
  // lyj add
  // GetPacket()->Print(std::cout);
  //NS_LOG_LYJ("packet format mark");
  UDPecn tag;
  Ptr<Packet> p = GetPacket ();

  if(p->RemovePacketTag (tag)){
    NS_LOG_LYJ("befor set: "<<(uint32_t)tag.GetECN());
    tag.SetECN (1);
    // std::cout<<" ecn node "<<GetAddress()<<std::endl;
    NS_LOG_LYJ("after set"<< (uint32_t)tag.GetECN());
    p->AddPacketTag (tag);
  }
