...
TypeId FifoQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FifoQueueDisc")
    ...
    .AddAttribute ("ECNThreshold",
                   "The max queue size",
                   UintegerValue (100),
                   MakeUintegerAccessor (&FifoQueueDisc::ecn_threshold),
...
FifoQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if (GetCurrentSize () + item > GetMaxSize ())
    {
      //NS_LOG_LYJ ("Queue full -- dropping pkt");
      DropBeforeEnqueue (item, LIMIT_EXCEEDED_DROP);
      return false;
    }
  
  if(GetNPackets() > ecn_threshold){
    NS_LOG_LYJ ("mark"<<GetNPackets());
    item->Mark();
    NS_LOG_LOGIC("cur size exceeds threshold and is "<<GetCurrentSize ());
  }
...
FifoQueueDisc::DoDequeue (void)
{
  ...
  //NS_LOG_LYJ("deque cur size is "<<GetCurrentSize ());
  return item;
}
