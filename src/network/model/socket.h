...
// for application layer's ecn mark
//lyj add

class UDPecn : public Tag
{
public:
  UDPecn ();
  ...
  virtual void Print (std::ostream &os) const;
private:
  uint8_t m_udpecn;  //!< the priority carried by the tag
};

//lzyadd for ecmp route

// struct SwmlRouteStruct{
//   uint16_t app_id;
//   uint16_t host_id;
// };


class SwmlRouteTag : public Tag
{
public:
  SwmlRouteTag ();
   ...
   * \param priority the priority
   */
  void SetSwmlRouteTag (  uint16_t appid,  uint16_t hostid);
   ...
   * \returns the priority
   */
  void GetSwmlRouteTag (uint16_t &appid,  uint16_t &hostid) const;
